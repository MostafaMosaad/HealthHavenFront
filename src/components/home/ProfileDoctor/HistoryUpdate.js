...
const handleChange = useCallback((e) => {
  ...
  const handleSubmit = (event) => {
    ...
    const fetchData = async () => {
      ...
      axios.patch(
        `http://localhost:3000/api/doctors/booking`,
        {
          PatientID: id,
          time:dte,
  ...
  return (
    <>
       <div>
       <form id="upForm" onSubmit={handleSubmit}>
       <div className="mb-2 d-flex">
        ...
        <button type="submit" className="saveBtnUp">{i18n.t("Update")}</button>
        <button type="submit" className="saveBtnUp"  onClick={handleBackClick}>{i18n.t("Back")}</button>
        </div>
        ...
        <input type = "datetime-local"   
                 ...
                 min={formattedToday2}
                 style={{borderColor:"red",padding:"0.5rem",borderRadius:"0.3rem",marginTop:"1rem",marginBottom:"1rem"}}
                 /> 
        </div>
       </form>
